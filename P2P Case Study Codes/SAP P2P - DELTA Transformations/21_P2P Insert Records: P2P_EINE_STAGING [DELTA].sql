...
DELETE FROM P2P_EINE_STAGING
WHERE EXISTS (SELECT 1 FROM <$EINE> as NEW_DATA 
              WHERE P2P_EINE_STAGING.MANDT=NEW_DATA.MANDT 
              AND P2P_EINE_STAGING.INFNR=NEW_DATA.INFNR
              AND P2P_EINE_STAGING.EKORG=NEW_DATA.EKORG
              AND P2P_EINE_STAGING.ESOKZ=NEW_DATA.ESOKZ
               AND P2P_EINE_STAGING.WERKS<=>NEW_DATA.WERKS
              );

INSERT INTO P2P_EINE_STAGING 
SELECT DISTINCT
     EINE.MANDT 
    ,EINE.INFNR
    ,EINE.EKORG	
...
FROM 
    <$EINE> AS EINE
    INNER JOIN P2P_EKPO_STAGING AS CASES ON 1=1
